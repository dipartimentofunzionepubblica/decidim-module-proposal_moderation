...
describe Decidim::ProposalModeration::Permissions do
  ...
  context "when permission is allowed" do
    context "when admin publish/reject comment" do
      subject { described_class.new(admin, permission_action, context).permissions.allowed? }
      let(:action) { { scope: :public, action: :publish, subject: :comment } }
      it { is_expected.to eq true }
    end
  end

  context "when permission is disallowed" do
    context "when not admin publish/reject comment" do
      let(:action) { { scope: :public, action: :publish, subject: :comment } }
      it_behaves_like "permission is not set"
    end

    context "when update review proposal" do
      let(:action) { { scope: :public, action: :update, subject: :proposal } }
