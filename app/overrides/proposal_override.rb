  ...
  module Proposals
    class Proposal
      ...
      scope :in_moderation_rejected_for_user, -> (user) do
        where({state: "moderation_rejected"}.merge(user.admin? ? {} : { id: Proposal.user_collection(user).ids }))
      end
      ...
      scope :not_in_moderation_rejected_for_user, -> (user) do
        ...
        where(state: "moderation_rejected").where.not(id: Proposal.in_moderation_rejected_for_user(user).ids)
      end
